...
import { ENTITY_SCHEDULES, ENTITY_MEMOS } from '../constants/firebase';
...
class FirebaseDatabaseService {
  ...
  async get_schedules(user_id: string, start_date?: string, end_date?: string): Promise<Schedule[]> {
    try {
      let q = query(
  ...
  async update_schedule(id: string, schedule: Partial<Schedule>): Promise<void> {
    try {
  ...
  // 메모 관련 메서드
  async create_memo(memo: Omit<Memo, 'id'>): Promise<string> {
    try {
